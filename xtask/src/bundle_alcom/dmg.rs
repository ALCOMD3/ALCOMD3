...
pub fn create_dmg(ctx: &BundleContext<'_>) -> anyhow::Result<()> {
    let app_bundle_name = ctx.app_bundle_name();
    let app_bundle = ctx.bundle_dir.join("macos").join(&app_bundle_name);
    let arch = target_arch(ctx.target_tuple);
    let volume_name = ctx.product_name();
    let dmg_name = format!("{volume_name}_{}_{arch}.dmg", ctx.version());
    ...
    //
    // Layout: the app bundle on the left, Applications symlink on the right.
    let ds_store_path = staging.join(".DS_Store");
    dmg_ds_store(volume_name, &app_bundle_name)
...
pub fn dmg_ds_store(volume_name: &str, app_name: &str) -> DsStore {
