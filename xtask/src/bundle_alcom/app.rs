...
/// Creates `bundle/macos/ALCOMD3.app`.
pub fn create_app_bundle(ctx: &BundleContext<'_>) -> Result<()> {
    let app_bundle_name = ctx.app_bundle_name();
    let app_dir = ctx.bundle_dir.join("macos").join(&app_bundle_name);
...
pub fn create_app_tar_gz(ctx: &BundleContext<'_>) -> Result<()> {
    ...
    let out = ctx
        .bundle_dir
        .join("macos")
        .join(format!("{app_bundle_name}.tar.gz"));
    create_tar_gz(&app_bundle, &app_bundle_name, &out)
}
