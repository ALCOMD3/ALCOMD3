...
use crate::utils::{cargo, download_file_cached, download_file_cached_sha256, target_abi};
...
// Digest published by the GitHub Release asset API for is-6_7_1.
const INNO_SETUP_INSTALLER_SHA256: &str =
...
pub fn create_setup_exe_zip(ctx: &BundleContext<'_>) -> Result<()> {
    ...
    zip.start_file(ctx.installer_file_name(), FileOptions::DEFAULT)
...
pub fn create_updater_exe(ctx: &BundleContext<'_>) -> Result<()> {
    ...
    // copy to bundle dir
    let wrapper_in_bundle = ctx
        .bundle_dir
        .join("setup")
        .join(format!("{}-updater.exe", ctx.package_name()));
...
fn setup_exe_path(ctx: &BundleContext<'_>) -> PathBuf {
    ctx.bundle_dir.join("setup").join(ctx.installer_file_name())
}
...
fn install_inno_setup(ctx: &BundleContext<'_>, runner: &WineRunner) -> Result<PathBuf> {
    ...
    download_file_cached_sha256(
        INNO_SETUP_INSTALLER_URL,
        &installer_exe,
        INNO_SETUP_INSTALLER_SHA256,
        "downloading Inno Setup installer",
    )?;

    if iscc.is_file() {
        // The cached compiler is reused only after its source installer is verified.
        return Ok(iscc);
    }
...
fn build_inno_setup_installer(
    ...
    cmd.arg(runner.path(&ctx.gui_dir.join("bundle/windows-setup.iss")))
        ...
        .arg(format!(
            "-DApplicationVersionInfoVersion={}",
...
fn windows_version_info_version() -> String {
    let version = cargo::gui_version();
...
fn build_wrapper(ctx: &BundleContext<'_>, libs_dir: &Path, iss_setup: &Path) -> Result<PathBuf> {
    ...
    Ok(ctx
        .build_dir
        .join(format!("{}-setup.exe", ctx.package_name())))
}
