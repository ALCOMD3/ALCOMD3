...
} from "./-bulk-update-range-selection";
import type {
	PackageLatestInfo,
	PackageRowInfo,
...
export const PackageListCard = memo(function PackageListCard({
	...
	const clearBulkUpdateSelection = useCallback(() => {
		setBulkUpdatePackageIds([]);
		lastSelectedPackageIdRef.current = null;
	}, []);

	useDocumentEvent("post-package-changes", clearBulkUpdateSelection, [
		clearBulkUpdateSelection,
	]);

	// biome-ignore lint/correctness/useExhaustiveDependencies(search): watched to reset the selection anchor when filtering changes
	// biome-ignore lint/correctness/useExhaustiveDependencies(packageRowsData): watched to reset the selection anchor when package or repository filtering changes
	useEffect(() => {
		lastSelectedPackageIdRef.current = null;
	...
	const onBulkUpdateCheckboxClick = useEffectEvent(
		(row: PackageRowInfo, shiftKey: boolean) => {
			...
			const anchorPackageId = lastSelectedPackageIdRef.current;

			if (shiftKey && anchorPackageId != null) {
				const visiblePackageRows = collectVisiblePackageRows(
					packageRowsData,
					hiddenPackages,
					filteredPackageIds,
					showHiddenPackages,
				);
				const rangePackageRows = collectPackageRowsInRange(
					visiblePackageRows,
					anchorPackageId,
					row.id,
					(rangeRow) =>
						canBulkUpdate(bulkUpdateMode, bulkUpdateModeForPackage(rangeRow)),
				);

				if (rangePackageRows != null) {
					setBulkUpdatePackageIds((previousPackageIds) =>
						applyPackageRangeSelection(
							previousPackageIds,
							rangePackageRows,
							nextChecked,
		...
		<Card className="grow shrink flex shadow-none w-full">
			<CardContent className="w-full p-2 flex flex-col gap-2 compact:p-1 compact:gap-1.5">
				...
				<BulkUpdateCard
					...
					packageRowsData={packageRowsData}
					cancel={clearBulkUpdateSelection}
				/>
				<ScrollableCardTable
	...
	const setShowPrereleasePackages = useMutation({
		...
		onSettled: async () => {
			await Promise.all([
				...
				queryClient.invalidateQueries({
					queryKey: ["projectPackageRows", projectPath],
				}),
			]);
		},
	});
...
const PackageRow = memo(function PackageRow({
	...
	const onClickBulkUpdateCheckbox = (event: React.MouseEvent) => {
		onBulkUpdateCheckboxClick(pkg, event.shiftKey);
	};
