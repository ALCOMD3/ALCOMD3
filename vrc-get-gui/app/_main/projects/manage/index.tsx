...
import { startProjectBackup } from "@/components/BackupProjectDialog";
...
import { collectPackageRowsFromBackend } from "./-collect-package-row-info";
...
type ProjectPackageChangedEvent = {
	projectPath: string;
};
...
function PageBody() {
	...
	const [
		...
		queries: [
			...
			},
			{
				...
				queryFn: () => commands.projectPackageRows(projectPath),
				refetchOnWindowFocus: false,
			},
			{
	...
	const packageRowsData = useMemo(() => {
		return collectPackageRowsFromBackend(packageRowsResult.data);
	}, [packageRowsResult.data]);

	const queryClient = useQueryClient();

	useTauriListen<ProjectPackageChangedEvent>(
	...
	const isLoading =
		packageRowsResult.isFetching ||
		detailsResult.isFetching ||
		repositoriesInfo.isFetching ||
		defaultRepositoriesResult.isFetching ||
...
function ProjectButton({
	...
	const onChangeLaunchOptions = async () => {
		...
		const settings = await openSingleDialog(
			LaunchSettings,
			{
