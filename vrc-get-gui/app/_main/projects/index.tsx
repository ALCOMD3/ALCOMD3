...
import { restoreProjectFromBackup } from "@/components/RestoreProjectFromBackupDialog";
...
function EmptyProjectsCard({
	...
	const importLegacyProjects = useMutation({
		mutationFn: async () => {
			const results: TauriLegacyDataImportResult[] = [];
			for (const source of legacySources.data ?? []) {
				results.push(
					await commands.environmentImportLegacyData(source.kind, "Projects"),
				);
			}
	...
	const startImportProjects = () => {
		if (legacySources.data == null || legacySources.data.length === 0) {
			toastError(tt("legacy import:no sources"));
			return;
		}

		importLegacyProjects.mutate();
	};

	return (
		<div className="flex h-full w-full items-center justify-center overflow-auto p-4">
			<Card className="flex w-full max-w-xl flex-col gap-4 border border-border p-6 compact:p-4">
				<div className="flex flex-col gap-1 text-center">
					<h2 className="text-lg font-medium">{tc("projects:empty:title")}</h2>
					<p className="whitespace-normal text-sm opacity-70">
						{tc("projects:empty:description")}
					</p>
				</div>
				<div className="flex flex-col justify-center gap-2 sm:flex-row">
					...
					<Button
						className="gap-2"
						disabled={legacySources.isLoading || importLegacyProjects.isPending}
						onClick={startImportProjects}
						variant="secondary"
					>
						<Download className="size-4" />
						{tc("projects:empty:import from vcc")}
					</Button>
				</div>
			</Card>
		</div>
...
function ProjectViewHeader({
	...
	const startRestoreProjectFromBackup = () => {
		void restoreProjectFromBackup(queryClient);
	};
			...
			trailing={
				<DropdownMenu>
					...
					<DropdownMenuContent>
						...
						<DropdownMenuItem onClick={startRestoreProjectFromBackup}>
