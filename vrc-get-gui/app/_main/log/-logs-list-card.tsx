			...
			{isLongMessage ? (
				<td className={`${cellClass} min-w-32 w-full`}>
					<details className={"group/log-text"}>
						<summary
							className={
								"whitespace-pre-wrap break-words list-none cursor-pointer"
							}
						>
							{firstMessageLine}
							<div
								className={
									"group-open/log-text:hidden mt-[-0.6lh] mb-[-0.4lh] compact:mb-[-0.2lh]"
								}
							>
								...
							</div>
						...
						<p className={"whitespace-pre-wrap break-words"}>
							{remainingMessageLines.join("\n")}
						</p>
					</details>
				</td>
			) : (
				<td className={`${cellClass} min-w-32 w-full whitespace-pre-wrap`}>
					{log.message}
				</td>
			)}
		</>
