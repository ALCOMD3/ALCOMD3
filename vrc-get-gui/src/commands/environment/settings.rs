    ...
    activity
        .track_result(
            ...
            ActivityInput::new(
                ...
                "Changing VCC protocol handling",
            )
            .target("useAlcomForVccProtocol")
            .details(vec![ActivityDetail::new(
                "value",
                use_alcom_for_vcc_protocol.to_string(),
            )]),
            "VCC protocol handling changed",
            ...
            async move {
                ...
                if use_alcom_for_vcc_protocol {
                    ...
                    spawn(crate::deep_link_support::deep_link_uninstall_vcc(
                        app_for_task,
                    ));
                }
                Ok(())
...
mod tests {
    #[test]
    fn disabling_automatic_download_keeps_an_already_downloaded_update() {
        let source = include_str!("settings.rs");
        let automatic_update = source
            .find("pub async fn environment_set_automatic_update")
            .unwrap();
        let next_command = source[automatic_update..]
            .find("pub async fn environment_set_use_alcom_for_vcc_protocol")
            .unwrap()
            + automatic_update;

        assert!(!source[automatic_update..next_command].contains("discard_staged_update"));
    }
}
