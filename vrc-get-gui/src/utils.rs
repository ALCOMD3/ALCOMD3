...
use vrc_get_vpm::io::{ALCOMD3_DATA_DIR_NAME, DefaultProjectIo};
...
fn documents_dir_or_home() -> PathBuf {
    dirs_next::document_dir().unwrap_or_else(home_dir)
}

pub(crate) fn default_backup_path() -> String {
    let mut documents = documents_dir_or_home();
    documents.extend(&[ALCOMD3_DATA_DIR_NAME, "Backups"]);
...
pub(crate) fn default_default_project_path() -> String {
    let mut documents = documents_dir_or_home();
    documents.extend(&[ALCOMD3_DATA_DIR_NAME, "Projects"]);
    documents.to_string_lossy().into_owned()
}
...
mod tests {
    ...
    #[test]
    fn default_project_path_lives_under_documents_alcomd3() {
        let mut expected =
            dirs_next::document_dir().unwrap_or_else(|| dirs_next::home_dir().unwrap());
        expected.extend([ALCOMD3_DATA_DIR_NAME, "Projects"]);

        assert_eq!(PathBuf::from(default_default_project_path()), expected);
    }

    #[test]
    fn default_backup_path_lives_under_documents_alcomd3() {
        let mut expected =
            dirs_next::document_dir().unwrap_or_else(|| dirs_next::home_dir().unwrap());
        expected.extend([ALCOMD3_DATA_DIR_NAME, "Backups"]);

        assert_eq!(PathBuf::from(default_backup_path()), expected);
    }
}
