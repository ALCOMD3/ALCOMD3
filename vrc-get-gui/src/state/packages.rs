            ...
            tokio::spawn({
                ...
                async move {
                    ...
                    let activity_tracker =
                        app_handle.try_state::<ActivityLogState>().map(|activity| {
                            activity.start_activity(
                                ...
                                ActivityInput::new(
                                    ...
                                    "Refreshing package cache in background",
                                ),
                            )
                        });
                        ...
                        && let Some(activity) = app_handle.try_state::<ActivityLogState>()
                    {
                        activity.finish_info(
                            Some(&app_handle),
                            activity_tracker,
                            "Package cache refresh attempt finished",
                            vec![ActivityDetail::new(
                                "packages",
                                arc.data.get().packages.len().to_string(),
                            )],
                        );
                    }
