...
const IMPORTED_VCC_TEMPLATE_PREFIX: &str = "com.alcomd3.imported.vcc.";
...
pub async fn load_resolve_all_templates(
    ...
    Ok(load_resolve_alcom_templates(io, unity_versions).await)
}
...
pub async fn load_resolve_alcom_templates(
    ...
    while {
        ...
        keys_to_update.retain(|k| {
            ...
            let Some(base_id) = alcom.base.as_ref() else {
                return false;
            };
            let Some(base) = template_by_id.get(base_id) else {
...
pub fn imported_vcc_template_id(name: &str) -> String {
    ...
    let safe = if safe.is_empty() { "template" } else { safe };
    let safe = safe.chars().take(50).collect::<String>();
    let safe = safe.trim_matches(['.', '_', '-']);
    let safe = if safe.is_empty() { "template" } else { safe };
    let digest = Sha256::digest(name.as_bytes());
    let hash = hex::encode(&digest[..6]);

    format!("{IMPORTED_VCC_TEMPLATE_PREFIX}{safe}.{hash}")
}

pub fn sanitize_template_file_stem(name: &str) -> String {
    ...
    let file_name = file_name.trim().trim_end_matches('.');
...
pub async fn load_alcom_templates(io: &DefaultEnvironmentIo) -> Vec<(PathBuf, AlcomTemplate)> {
    let path = Path::new(crate::storage::TEMPLATE_DIR);
    let mut dir = match io.read_dir(path).await {
        Ok(dir) => dir,
        Err(ref e) if e.kind() == io::ErrorKind::NotFound => return Vec::new(),
        Err(e) => {
            warn!("failed to read ALCOMD3 templates directory {path:?}: {e}");
            return Vec::new();
        }
    };
...
pub async fn create_project_archive_template(
    source_path: &Path,
    display_name: &str,
    id: String,
) -> io::Result<Vec<u8>> {
    let unity_project = UnityProject::load(DefaultProjectIo::new(source_path.into())).await?;
    let source_path = source_path.to_path_buf();
    let archive_payload =
        tokio::task::spawn_blocking(move || archive_directory_to_tar_gz(&source_path))
            .await
    ...
    let template = AlcomTemplate {
        ...
        archive: Some(AlcomTemplateArchive {
            format: AlcomTemplateArchiveFormat::TarGz,
            unity_version: unity_project.unity_version(),
        }),
    };

    serialize_alcom_project_archive_template(template, &archive_payload).map_err(io::Error::other)
}

fn archive_directory_to_tar_gz(source_path: &Path) -> io::Result<Vec<u8>> {
    let encoder = GzEncoder::new(Vec::new(), Compression::default());
    let mut builder = tar::Builder::new(encoder);
    append_directory_contents(&mut builder, source_path, Path::new(""))?;
    builder.finish()?;
...
fn append_directory_contents<W: io::Write>(
    ...
    for entry in std::fs::read_dir(source_path)? {
        ...
        let path = entry.path();
        let archive_entry_path = archive_path.join(entry.file_name());

        if file_type.is_dir() {
            builder.append_dir(&archive_entry_path, &path)?;
            append_directory_contents(builder, &path, &archive_entry_path)?;
        } else if file_type.is_file() {
            builder.append_path_with_name(&path, &archive_entry_path)?;
        } else {
            warn!(
                "skipping unsupported template archive entry: {}",
                path.display()
            );
        }
    }
...
fn extract_project_archive_payload(
    archive_payload: &[u8],
    project_path: &Path,
    format: AlcomTemplateArchiveFormat,
) -> io::Result<()> {
    match format {
        AlcomTemplateArchiveFormat::TarGz => {
            let decoder = flate2::read::GzDecoder::new(io::Cursor::new(archive_payload));
            let mut archive = tar::Archive::new(decoder);
            for entry in archive.entries()? {
                let mut entry = entry?;
                entry.unpack_in(project_path)?;
            }
        }
    }
...
pub async fn create_project(
    ...
    impl ResolvedTemplateInfo {
        ...
        fn project_archive(source_path: PathBuf, format: AlcomTemplateArchiveFormat) -> Self {
            Self {
                base_template: BaseTemplate::ProjectArchive {
                    source_path,
                    format,
                },
                packages: IndexMap::new(),
    ...
    match template_info.base_template {
        ...
        BaseTemplate::ProjectArchive {
            source_path,
            format,
        } => {
            let mut file = io.open(&source_path).await?;
            let mut buffer = Vec::new();
            file.read_to_end(&mut buffer).await?;
            let payload = alcom_template_project_archive_payload(&buffer)
                .ok_or_else(|| io::Error::other("project archive template has no payload"))?
                .to_vec();
            let project_path = project_path.to_path_buf();
            tokio::task::spawn_blocking(move || {
                extract_project_archive_payload(&payload, &project_path, format)
            })
            .await
            .map_err(|_| io::Error::other("background task failed"))??;
        }
    ...
    fn resolve_template<'a>(
        ...
        match id {
            ...
            // .alcomtemplate files
            id => {
                let template_info = templates.get(id)?;
                let template = template_info
                    .alcom_template
                    .as_ref()
                    .expect("no .alcomtemplate info");
                if let Some(archive) = &template.archive {
                    return Some(ResolvedTemplateInfo::project_archive(
                        template_info.source_path.as_ref()?.clone(),
                        archive.format,
                    ));
                }

                let mut resolved =
                    resolve_template(templates, template.base.as_ref()?, unity_version)?;
...
mod tests {
    ...
    fn project_archive_template_round_trips_project_directory() {
        tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap()
            .block_on(project_archive_template_round_trips_project_directory_inner());
    }

    async fn project_archive_template_round_trips_project_directory_inner() {
        let temp = tempfile::tempdir().unwrap();
        let source = temp.path().join("source");
        tokio::fs::create_dir_all(source.join("Assets"))
            .await
            .unwrap();
        tokio::fs::create_dir_all(source.join("Packages"))
            .await
            .unwrap();
        tokio::fs::create_dir_all(source.join("ProjectSettings"))
            .await
            .unwrap();
        tokio::fs::write(source.join("package.json"), "{}")
            .await
            .unwrap();
        tokio::fs::write(source.join("Assets/Keep.txt"), "kept")
            .await
            .unwrap();
        tokio::fs::write(
        ...
        let id = imported_vcc_template_id("Legacy VCC Template");
        let template = create_project_archive_template(&source, "Legacy VCC Template", id.clone())
            .await
            .unwrap();
        let parsed = parse_alcom_template(&template).unwrap();

        assert!(parsed.is_project_archive());
        assert_eq!(
            parsed.archive.unwrap().unity_version,
            UnityVersion::new_f1(2022, 3, 22)
        );

        let environment = temp.path().join("environment");
        let template_dir = environment.join(crate::storage::TEMPLATE_DIR);
        tokio::fs::create_dir_all(&template_dir).await.unwrap();
        tokio::fs::write(template_dir.join("legacy.alcomtemplate"), template)
            .await
            .unwrap();

        let io = DefaultEnvironmentIo::new(environment.into_boxed_path());
        let templates = load_resolve_all_templates(&io, &[]).await.unwrap();
        let imported = templates.iter().find(|template| template.id == id).unwrap();

        assert!(imported.available);
        assert_eq!(
            imported.unity_versions,
            vec![UnityVersion::new_f1(2022, 3, 22)]
        );

        let project_path = temp.path().join("created");
        ...
        assert_eq!(
            tokio::fs::read_to_string(project_path.join("Assets/Keep.txt"))
                .await
                .unwrap(),
            "kept"
        );
