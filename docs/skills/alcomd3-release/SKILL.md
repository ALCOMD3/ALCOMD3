...
- 验证 GitHub Release 的多平台资产与公开 updater endpoint；
- 审计发布失败或 updater metadata 上线状态。
...
6. 目标 channel 的 updater JSON 已原子更新为三个平台、推送到配置的 metadata 仓库，并与 public updater endpoint 完全一致。
...
7. 使用专用 deploy key 克隆配置的 metadata 仓库；
...
公开 Release 之后，endpoint 暂未更新不是回滚理由。修复 metadata 托管问题并重跑 updater
workflow；不要改写已公开 Release 资产。
...
- 10 资产清单与 manifest/digest 校验结果；
- metadata 仓库 commit、目标 JSON、三平台签名验证与 public endpoint；
