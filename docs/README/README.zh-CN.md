# ALCOMD3 documentation

语言: [English](../README.md) | [日本語](README.ja.md) | 简体中文

这是 ALCOMD3 的文档入口。需要定位项目规则、维护笔记、发布步骤或用户可见技术文档时，从这里开始。

### 首先阅读

- 项目概览和下载：[../../README/README.zh-CN.md](../../README/README.zh-CN.md)
- 贡献规则：[../../CONTRIBUTING/CONTRIBUTING.zh-CN.md](../../CONTRIBUTING/CONTRIBUTING.zh-CN.md)
- 许可证说明：[../../LICENSE-NOTES.md](../../LICENSE-NOTES.md)
- Agent 规则：[../../AGENTS.md](../../AGENTS.md)

### 维护者文档

- 维护笔记和兼容性边界：[MAINTENANCE.zh-CN.md](../MAINTENANCE/MAINTENANCE.zh-CN.md)
- 持续维护的测试链路：[TESTING.zh-CN.md](../TESTING/TESTING.zh-CN.md)（[English](../TESTING.md)）
- 跨平台 full-chain workflow：[../../.github/workflows/full-chain.yml](../../.github/workflows/full-chain.yml)
- 发布流程：[RELEASE.zh-CN.md](../RELEASE/RELEASE.zh-CN.md)
- 默认 Draft 构建 workflow：[../../.github/workflows/release-draft.yml](../../.github/workflows/release-draft.yml)
- 发布后 updater workflow：[../../.github/workflows/release-updater.yml](../../.github/workflows/release-updater.yml)
- Updater 签名和验证：[ALCOMD3_UPDATER.zh-CN.md](../ALCOMD3_UPDATER/ALCOMD3_UPDATER.zh-CN.md)
- 产品身份和发布共享配置：[../../alcomd3.config.json](../../alcomd3.config.json)
- Agent skill 索引：[skills/README.md](../skills/README.md)
- Agent 发布 skill：[skills/alcomd3-release/SKILL.md](../skills/alcomd3-release/SKILL.md)

### 功能与格式文档

- MCP bridge 指南：[mcp.zh-CN.md](../mcp/mcp.zh-CN.md)（[English](../mcp.md)，[日本語](../mcp/mcp.ja.md)，[繁體中文](../mcp/mcp.zh-TW.md)）
- Repository list 文件格式：[repository-list-file-format.zh-CN.md](../repository-list-file-format/repository-list-file-format.zh-CN.md)（[English](../repository-list-file-format.md)，[日本語](../repository-list-file-format/repository-list-file-format.ja.md)）
- 默认 repository list 示例/导入数据：[../../repositories.txt](../../repositories.txt)
  （`repositories.txt` 不是运行时共享配置。）

### 历史记录

- Release notes：[../../release-notes/](../../release-notes/)

### 子项目文档

- GUI README：[../../vrc-get-gui/README.md](../../vrc-get-gui/README.md)
- GUI 贡献指南：[../../vrc-get-gui/CONTRIBUTING/CONTRIBUTING.zh-CN.md](../../vrc-get-gui/CONTRIBUTING/CONTRIBUTING.zh-CN.md)
- GUI 第三方声明：[../../vrc-get-gui/THIRD-PARTY.md](../../vrc-get-gui/THIRD-PARTY.md)
- GUI 项目模板：[../../vrc-get-gui/project-templates/README.md](../../vrc-get-gui/project-templates/README.md)
- Windows 安装器辅助程序：[../../vrc-get-gui/windows-installer-wrapper/](../../vrc-get-gui/windows-installer-wrapper/)
- VPM library README：[../../vrc-get-vpm/README.md](../../vrc-get-vpm/README.md)
