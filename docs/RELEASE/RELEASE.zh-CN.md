...
| --- | --- | --- | --- |
| Stable | `3.0.0` | normal release | metadata 仓库 `public/api/gui/tauri-updater.json` |
| Beta | `3.1.0-beta.1` | prerelease | metadata 仓库 `public/api/gui/tauri-updater-beta.json` |
...
所有新公开文件名都显式包含平台和架构。历史 Release 资产不重命名、不改写。

在 GitHub UI 中人工发布 Draft。这是发布门，默认构建 workflow 不会绕过它。

### 6. 等待 updater workflow
...
metadata 是否创建新提交、最终 metadata 仓库 commit 和已验证的 endpoint。

Updater workflow 不接收私钥。Checkout 不持久化凭据，`GH_TOKEN` 只在必要 step 可见，
且会从非 GitHub cargo、git 子进程移除。

### 7. 确认完成与失败恢复

只有 **Publish updater metadata** 和公开 endpoint 检查都通过，发布才算完成。Workflow
会在有限时间内重试；超时不会回滚已经验证并推送的 metadata commit。修复 metadata 托管
问题后重跑 updater workflow。同一 Release 会重建出逐字节相同的 JSON，重跑会跳过无变化
的 commit/push 并继续检查 endpoint。
...
Updater metadata 发布仅允许在 Actions 中运行，避免绕过 source 绑定、版本
...
- GitHub Release assets 公开前试图发布 updater JSON。
