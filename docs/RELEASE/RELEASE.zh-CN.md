...
- `vrc-get-gui/package.json` 和 `website/package.json` 由
  `cargo xtask release-prepare` 更新。
...
- 运行 Website check/build；官网从目标 channel manifest 与 `alcomd3.config.json` 派生
  三平台下载，保持另一 channel 不变，且不为 legacy 资产合成链接；
- Website 通过后才提交该 updater JSON 并推送 `main`；
- 等待公开 updater endpoint 返回同一版本、三个精确平台 URL 和签名。

公开 endpoint 通过后，Job Summary 会记录 version、channel、Release 与 source commit、
metadata 是否创建新提交、最终 `main` commit 和已验证的 endpoint。

Updater workflow 不接收私钥。Checkout 不持久化凭据，`GH_TOKEN` 只在必要 step 可见，
且会从非 GitHub cargo、npm、git 子进程移除。它对 `main` 的 push 会触发已连接的
Cloudflare Pages production deployment，因此应保持 `main` 的自动 production branch
deployment 开启。
...
- Cloudflare Pages 对 `main` 的 production 自动部署被关闭；
