...
Cloudflare Pages deployment、public endpoint の監視を続け、すべて成功した場合だけ release
complete とする。
...
manifest を使う。Stable 2.1.1 は immutable historical GitHub Release として保持し、
Website は legacy assets から direct links を導出しない。Beta は引き続き別の選択肢として
明示し、Website は channel を推奨しない。ブラウザーで platform を識別できる場合は、
対応する stable package card だけを強調し、Beta package は強調しない。
...
- GitHub Immutable Releases を有効にする。
- `ALCOMD3_UPDATER_PRIVATE_KEY` と
  `ALCOMD3_UPDATER_PRIVATE_KEY_PASSWORD` を repository Actions Secrets として保存する。
- `main` の Cloudflare Pages production automatic deployment を有効に保つ。
...
Draft workflow を dispatch する前に、repository Administration 権限を持つ token の
`gh api` preflight で Immutable Releases を確認する。job-scoped `GITHUB_TOKEN` には
必要な read permission がないため、workflow 内ではこの read を意図的に繰り返さない。
published updater metadata は引き続き immutable Release attestation により gate される。
...
- GUI と website の npm versions を tag なしで更新する。
- npm lockfiles を更新する。
...
置く。各 locale は 1 段落の概要から始め、アプリの更新、Web サイトの更新、インストールと
アップグレード、互換性とセキュリティの 4 つの level-3 category をこの順で必ず保持する。
Localized title はそれぞれ `Application updates` / `アプリの更新` / `应用更新`、
`Website updates` / `Web サイトの更新` / `网站更新`、`Installation and upgrade` /
...
review で確認する。
`2.1.3-beta.2` 以前の公開済み notes は historical heading を保持し、新しい形式に合わせて
書き換えない。固定 4 category contract は後続の release から適用する。
...
すべての new public filename は platform と architecture を明示する。Stable 2.1.1 は
immutable historical GitHub Release として保持する。Legacy Windows asset names は new
catalog に含めず、Website direct download links や compatibility aliases に変換しない。

GitHub UI で Draft を手動 publish する。これは release gate であり、既定の build
workflow は bypass しない。

### 6. Updater workflow と Cloudflare Pages を待つ
...
- Immutable Release attestation を検証し、10 public assets をすべて download して、
  各 local asset が attestation と一致することを検証する。
...
complete。Cloudflare Pages が deployment 中なら workflow は有限回 retry する。Timeout は
検証済み metadata commit を rollback しない。Pages deployment を確認・修復し、endpoint
workflow を再実行する。同じ Release は byte-identical JSON を再生成するため、変更なしの
...
Updater metadata publication は Actions-only とし、attestation、source binding、monotonic
...
- Immutable Releases が無効、または Release/asset attestation 検証が失敗する。
