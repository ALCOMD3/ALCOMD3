...
`2.1.2-beta.1` 首次在 beta metadata 采用这套三平台合约；stable `2.1.2` 采用同一合约。
Stable 2.1.1 继续作为不可变的历史 Release 保留，官网不为其旧文件名合成直接下载链接或
兼容别名。
...
人工发布 Draft 后，`release-updater.yml` 会下载全部 10 项公开资产，并验证每项 Immutable
Release asset attestation；随后验证三个 updater payload/signature pair，从 Release tag 读取
...
发布网站前必须验证最终 updater JSON。默认的 published updater workflow 会使用下载的
