...
mod windows {
    ...
    pub fn global_config_folder() -> PathBuf {
        program_data_path().joined("Unity\\config")
    }

    fn program_data_path() -> PathBuf {
        env_path("ALLUSERSPROFILE")
            .or_else(|| env_path("ProgramData"))
            .or_else(known_folder_program_data)
    ...
    fn known_folder_program_data() -> Option<PathBuf> {
        ...
        unsafe {
            let path = SHGetKnownFolderPath(&FOLDERID_ProgramData, KF_FLAG_DEFAULT, None).ok()?;
            if path.is_null() {
                return None;
            }

            // SHGetKnownFolderPath returns a CoTaskMem-allocated UTF-16 string.
    ...
    mod tests {
        ...
        #[test]
        fn global_config_folder_uses_program_data_when_allusersprofile_is_missing() {
            let _guard = ENV_LOCK.lock().unwrap();
            let _allusersprofile = EnvVarGuard::set("ALLUSERSPROFILE", None);
            let _program_data = EnvVarGuard::set("ProgramData", Some(r"C:\ProgramDataFromEnv"));

            assert_eq!(
                global_config_folder(),
                PathBuf::from(r"C:\ProgramDataFromEnv\Unity\config")
            );
        }
