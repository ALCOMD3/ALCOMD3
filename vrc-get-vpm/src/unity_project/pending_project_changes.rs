...
use crate::{PackageInfo, UnityProject, package_manifest_is_unity_compatible};
...
impl<'env> Builder<'env> {
    ...
    pub async fn build_resolve(
        ...
        for package in installs
            .iter()
            .filter(|pkg| {
                !package_manifest_is_unity_compatible(
                    pkg.package_json(),
                    unity_project.unity_version,
                )
            })
...
impl UnityProject {
    ...
    pub async fn apply_pending_changes_with_abort<'env, Env: PackageInstaller>(
        ...
        let successful_installs = installs
            ...
            .collect::<Vec<_>>();
        let finalize_abort = AbortCheck::new();
        ...
        let mut r: io::Result<()> = async {
            ...
            package_failures.extend(
                context
                    .move_uninstall_packages(&self.io, env, &finalize_abort, &uninstall_packages)
                    .await,
            );
            ...
            package_failures.extend(
                context
                    .move_install_packages(&self.io, env, &finalize_abort, &successful_installs)
                    .await,
            );
