...
use crate::environment::{
    ...
    OFFICIAL_URL_STR, PackageCollection,
};
...
impl Settings {
    ...
    pub fn can_add_remote_repo(&self, url: &Url, remote_repo: &RemoteRepository) -> bool {
        ...
        if !self.ignore_curated_repository() && url.as_str() == CURATED_URL_STR {
            return false;
        }
        if !self.ignore_official_repository() && url.as_str() == OFFICIAL_URL_STR {
            return false;
        }
        ...
        if let Some(repo_id) = remote_repo.id() {
            ...
            if repo_id == CURATED_REPOSITORY_ID && !self.vrc_get.ignore_curated_repository() {
                return false;
            }
        }
