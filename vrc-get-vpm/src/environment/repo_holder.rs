...
use crate::environment::{
    CURATED_URL_STR, LOCAL_CURATED_FILE, LOCAL_OFFICIAL_FILE, OFFICIAL_URL_STR, REPO_CACHE_FOLDER,
    Settings,
};
...
impl RepoHolder {
    ...
    fn get_predefined_repos(settings: &Settings) -> Vec<RepoSource<'static>> {
        lazy_static! {
            ...
            static ref LOCAL_OFFICIAL_PATH: PathBuf =
                Path::new(REPO_CACHE_FOLDER).join(LOCAL_OFFICIAL_FILE);
            static ref LOCAL_CURATED_PATH: PathBuf =
                Path::new(REPO_CACHE_FOLDER).join(LOCAL_CURATED_FILE);
        }
