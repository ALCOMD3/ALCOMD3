...
use crate::environment::VPM_SETTINGS_BACKUP_PATH;
use crate::io;
...
impl VpmSettings {
    ...
    pub async fn load_alt(io: &DefaultEnvironmentIo) -> io::Result<Option<Self>> {
        let mut settings = Self::load_inner(io, VPM_SETTINGS_BACKUP_PATH).await?;
    ...
    pub async fn save(&self, io: &DefaultEnvironmentIo) -> io::Result<()> {
        ...
        save_json(io, VPM_SETTINGS_BACKUP_PATH.as_ref(), &self.parsed).await
    }
}
...
mod tests {
    ...
    const LEGACY_VPM_SETTINGS_BACKUP_PATH: &str = "vrc-get/vcc-settings-backup.json";

    fn temp_environment() -> (DefaultEnvironmentIo, PathBuf) {
        let root = std::env::temp_dir().join(format!(
            "alcomd3-vpm-settings-test-{}",
    ...
    #[tokio::test]
    async fn load_alt_ignores_legacy_backup_path() {
        let (io, root) = temp_environment();
        let legacy_path = io.resolve(Path::new(LEGACY_VPM_SETTINGS_BACKUP_PATH));
        tokio::fs::create_dir_all(legacy_path.parent().unwrap())
            .await
            .unwrap();
        tokio::fs::write(
            &legacy_path,
            br#"{"defaultProjectPath":"C:/legacy/projects","projectBackupPath":"C:/legacy/backups"}"#,
        )
        .await
        .unwrap();

        let settings = VpmSettings::load_alt(&io).await.unwrap();
    ...
    #[tokio::test]
    async fn load_alt_reads_current_backup_path() {
        let (io, root) = temp_environment();
        let current_path = io.resolve(Path::new(VPM_SETTINGS_BACKUP_PATH));
        tokio::fs::create_dir_all(current_path.parent().unwrap())
            .await
            .unwrap();
        tokio::fs::write(
            &current_path,
            br#"{"defaultProjectPath":"C:/current/projects","projectBackupPath":"C:/current/backups"}"#,
        )
        .await
        .unwrap();

        let settings = VpmSettings::load_alt(&io).await.unwrap().unwrap();
