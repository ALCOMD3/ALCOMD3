...
pub use alcomd3_app_paths::{
    ALCOMD3_DATA_DIR_NAME, LEGACY_ALCOM_DATA_DIR_NAME, LEGACY_VCC_DATA_DIR_NAME,
};
...
impl DefaultEnvironmentIo {
    ...
    pub fn new_default() -> Self {
        let folder = alcomd3_app_paths::alcomd3_data_dir();
        ...
        DefaultEnvironmentIo::new(folder.into_boxed_path())
    }

    pub fn new_legacy_vcc() -> Self {
        Self::new_named_data_dir(LEGACY_VCC_DATA_DIR_NAME)
    }

    pub fn new_legacy_alcom() -> Self {
        Self::new_named_data_dir(LEGACY_ALCOM_DATA_DIR_NAME)
    }

    fn new_named_data_dir(name: &str) -> Self {
        let folder = alcomd3_app_paths::named_data_dir(name);
...
mod tests {
    ...
    fn default_environment_io_uses_alcomd3_data_directory() {
        ...
        assert_eq!(root.file_name().and_then(OsStr::to_str), Some("ALCOMD3"));
    }

    #[test]
    fn default_environment_io_uses_shared_alcomd3_data_directory() {
        let io = DefaultEnvironmentIo::new_default();
        let root = io.resolve(Path::new(""));
